...
| 模式 | 正确数 | 正确率 | 总耗时 |
|:-----|:------:|:------:|:------:|
| off（无审查） | 1/20 | 5% | 9295s |
| strict（审查） | 2/20 | 10% | 9281s |

> 当前结果受限于 DeepSeek v4 Flash 的推理速度和 180s 超时限制。复杂多轮任务（3 轮迭代 + 跨文件）需要更多推理步数，在更快的模型（如 DeepSeek v4 Pro 或 Qwen Max）上预期通过率会有显著提升。
