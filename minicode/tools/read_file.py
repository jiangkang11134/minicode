...
提供文件内容读取功能，支持偏移量和字符数限制，并带有文件内容缓存机制以避免重复读取。
"""
...
def _get_cached_file_content(target: Path) -> str:
    """获取文件内容，使用缓存避免重复读取同一文件。

    缓存键由文件路径和修改时间组成，缓存有效期为 2 秒。读取后自动清理过期缓存条目。
...
def _validate(input_data: dict) -> dict:
    ...
    参数:
        input_data: 包含 path、offset、limit 等字段的原始输入字典。

    返回:
        规范化后的参数字典，包含 path、offset、limit 键。
...
def _run(input_data: dict, context) -> ToolResult:
    ...
    解析文件路径，通过缓存读取文件内容，并根据 offset 和 limit 截取指定范围返回。
    如果文件为二进制无法解码，则返回错误信息。

    参数:
        input_data: 已验证的输入字典，包含 path、offset、limit。
