"""输入提示行渲染：显示 mini-code> 提示符和带光标的输入区域。

支持单行和多行（通过 Ctrl+J 换行）输入显示，多行时延续行使用缩进前缀。
光标使用高亮背景色 + 绿色字符展示。
"""
...
def render_input_prompt(current_input: str, cursor_offset: int, compact: bool = False) -> str:
    ...
    参数:
        ...
        cursor_offset: 光标偏移位置（字符索引）
        compact: 是否紧凑模式（仅渲染输入行，不含快捷键提示栏）
