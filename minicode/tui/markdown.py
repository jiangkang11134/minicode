...
def _highlight_code(line: str, lang: str = "") -> str:
    ...
    使用基于正则的简单分词器对代码行进行着色，支持注释、字符串、
    装饰器、数字、关键词、类型名、函数调用、运算符和标点符号的高亮。
    不带语言标记时仅使用暗色背景显示。

    注意：这不是完整的语法解析器，但对于终端显示已足够。

    参数:
        line: 单行代码文本
        lang: 编程语言标识符（如 'python'、'js' 等），空字符串时不进行语法高亮
...
def _inline_replace(m: Match) -> str:
    """作为单次正则替换回调，处理行内 Markdown 格式。

    匹配六种行内格式并返回对应的 ANSI 渲染结果：
      - 行内代码: 背景色 + 前景色
      - 粗体: BOLD 转义
      - 删除线: STRIKETHROUGH + DIM
      - 斜体: ITALIC 转义
      - 链接: 下划线 + 链接 URL 附在文本后

    参数:
        m: 正则匹配对象，来自 ``_RE_INLINE`` 的匹配结果
...
def render_markdownish(input_text: str) -> str:
    """将 Markdown 文本渲染为带终端格式和语法高亮的字符串。

    支持：
      - 围栏代码块（Fenced code blocks）带语言特定的语法高亮
      - 行内代码、粗体、斜体、删除线、链接
      - 标题 H1-H3 带视觉层级（H1 加下划线亮青色、H2 加下划线彩色、H3 加粗彩色）
      - 无序列表、有序列表和任务列表
      - 引用块带视觉竖线
      - 表格带对齐列
      - 水平分隔线

    结果通过内容哈希缓存以避免在帧间重复渲染，缓存上限为 300 条。

    参数:
        input_text: 原始 Markdown 文本
